(function () {
  ...
  function paint(list) {
    list = sortNew(filt(list || window.__baLastList || []));
    ...
    if (rp) {
      ...
      rp.innerHTML = list.length ? list.map(function (r) {
        var tot = Number(r.total) || ((Number(r.price)||0)*(Number(r.qty)||0));
      ...
      }).join('') : '<tr><td colspan="11" style="padding:1rem;text-align:center;color:#94a3b8">Tidak ada data</td></tr>';
    }
  }
  function save(rows) {
  ...
  function lockRenderer() {
    window.baRenderTable = function (list) {
      if (list && list.length) window.__baLastList = list;
      paint(window.__baLastList || list || []);
    };
  }
  lockRenderer();
  setInterval(lockRenderer, 800);
  ...
  document.addEventListener('click', function (e) {
    ...
    if (!el) return;
    if (/report|berita|filter/i.test(el.textContent || '')) setTimeout(function () { paint(window.__baLastList || []); }, 350);
  }, true);
  loadAll();
  hookRefresh();
